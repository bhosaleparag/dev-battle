...
export default function CodeEditor({ data }) {
  ...
  // Timer state
  const [timeLeft, setTimeLeft] = useState(data.timeLimit || 300); // seconds
  ...
  const getTimerColor = () => {
    const percentage = (timeLeft / (data.timeLimit || 300)) * 100;
    if (percentage > 50) return 'text-green-400 bg-green-400/20 border-green-400/30';
    if (percentage > 20) return 'text-yellow-400 bg-yellow-400/20 border-yellow-400/30';
    return 'text-red-400 bg-red-400/20 border-red-400/30 animate-pulse';
  };
    ...
    try {
      ...
      allResults = data.testCases.map((tc, i) => {
        ...
        try {
          ...
          passed = actual.toString() === tc.expectedOutput.toString();
        }
      ...
      const tempXP = calculateXP(passedCases, data.testCases.length, data.xp);
      
      setPercentage(Math.round((passedCases / data.testCases.length) * 100));
      setEarnedXP(tempXP);
      setTimerActive(false);
      
      await saveDebuggerResult(user.uid, data._id, completed, tempXP);
    ...
    <div className="min-h-[91vh] bg-gray-08 text-white">
          ...
          <div className="bg-gray-10 border-b border-gray-15 mb-0 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-15">
              ...
              <div className="flex items-center gap-3">
                <SoundButton
                  onClick={handleRun}
                  disabled={isRunning || !timerActive}
      ...
      {showResults && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-10 border border-gray-15 rounded-2xl max-w-2xl w-full p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <h2 className="text-3xl font-bold text-white mb-2">
                {passedTests === totalTests ? 'Challenge Complete!' : 'Keep Trying!'}
              </h2>
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-gray-15 border border-gray-20 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-gray-60 font-medium">XP Earned</span>
                  <span className="text-3xl font-bold text-purple-60">+{earnedXP}</span>
                </div>
                
                <div className="w-full bg-gray-30 rounded-full h-3 mb-2">
                  <div
                    className="bg-gradient-to-r from-purple-60 to-purple-75 h-3 rounded-full transition-all duration-500"
                    style={{ width: `${percentage}%` }}
                  />
                </div>
                <p className="text-sm text-gray-60 text-center">{percentage}% Score</p>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div className="bg-gray-15 border border-gray-20 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-emerald-400">{passedTests}</div>
                  <div className="text-xs text-gray-60 mt-1">Passed</div>
                </div>
                <div className="bg-gray-15 border border-gray-20 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-red-400">{totalTests - passedTests}</div>
                  <div className="text-xs text-gray-60 mt-1">Failed</div>
                </div>
                <div className="bg-gray-15 border border-gray-20 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-white">{totalTests}</div>
                  <div className="text-xs text-gray-60 mt-1">Total</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <SoundButton
                ...
                className="flex-1 px-6 py-3 bg-gray-20 hover:bg-gray-30 text-white rounded-xl font-medium transition-all"
              >
                Review Code
              </SoundButton>
              <SoundButton
                onClick={() => router.push('/explore')}
                className="flex-1 px-6 py-3 bg-purple-60 hover:bg-purple-70 text-white rounded-xl font-medium transition-all"
              >
                Next Challenge
              </SoundButton>
            </div>
